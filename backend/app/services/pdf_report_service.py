...
def generate_pdf(file_path, organisation_name, project_name, date_from, date_to, sections_data, skipped_sections):
    ...
    if "sentimentDistribution" in sections_data:
        ...
        story.append(_table(
            ...
            [
                ...
                ["Winning-label score (avg)", f"{s['averageConfidence']:.1%} — not a calibrated probability"],
            ],
        ))
    ...
    if "aiInterpretation" in sections_data:
        ...
        # Honest labelling is a contract: "AI-Generated" is reserved for
        # sections produced by a live LLM. The deterministic fallback is
        # labelled as a summary, explicitly not AI-generated.
        if source == "llm":
            source_label = "AI-Generated Contextual Interpretation (LLM)"
            notice = (
                "<i>This section was generated by a language model. It is not a measured finding. "
                "It is provided to help interpret the deterministic numbers above. "
                "Do not cite as a fact.</i>"
            )
        else:
            source_label = "Deterministic Contextual Summary"
            notice = (
                "<i>This section was produced deterministically from the verified metrics above. "
                "No language model was involved in producing it.</i>"
            )
        story.append(Paragraph(source_label, styles["SectionHeading"]))
        story.append(Paragraph(notice, styles["ReportMeta"]))
        # Escape reportlab markup in the AI-generated narrative. The text
        # is never trusted — it must render as literal text, not markup
        # (prevents injection of reportlab-specific tags and broken
        # output when the model emits XML-ish fragments).
        ai_text = _html_escape(meta.get("text", ""), quote=False)
        # html.escape escapes < > & but not newlines; Paragraph wraps
        # whitespace-collapsed text. Normalise: convert literal newlines
        # to <br/> so paragraph breaks survive visually.
        ...
        if meta.get("model"):
            meta_parts.append(f"Model: {meta.get('model')}")
        if meta.get("status"):
            meta_parts.append(f"Status: {meta.get('status')}")
        if meta.get("latencyMs") is not None:
            meta_parts.append(f"Latency: {int(meta.get('latencyMs') or 0)} ms")
        if meta.get("warning"):
            meta_parts.append(f"Note: {str(meta.get('warning'))[:200]}")
        if meta_parts:
